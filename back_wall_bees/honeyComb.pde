...
//    void addBees() {
//        int numBees = 16 + int(random(2)); // Add between 3 and 4 bees
...
//            float size = random(0.5, 1.5);  // Random size between 0.5 and 1.5
...
//    ArrayList<PVector> getHexagonEdgePoints() {
//        ArrayList<PVector> edgePoints = new ArrayList<PVector>();
//        for (Hexagon hex : hexs) {
//            edgePoints.addAll(hex.getEdgePoints());
//        }
//        return edgePoints;
//    }
//}
...
class HexagonGrid {
    ...
    void addBees() {
        int numBees = 20 + int(random(2)); // Add between 16 and 18 bees
        for (int i = 0; i < numBees; i++) {
            // Place bees within the bounds of the grid
            Hexagon targetHex = hexs.get(int(random(hexs.size())));
            float angle = random(TWO_PI); // Random angle within the hexagon
            float distance = random(radius * 0.9); // Random distance within the hexagon
            float offsetX = cos(angle) * distance;
            float offsetY = sin(angle) * distance;
            float size = random(0.5, 1.5);  // Random size between 0.5 and 1.5
            restingBees.add(new HoneyCombBee(targetHex.x + offsetX, targetHex.y + offsetY, size));
