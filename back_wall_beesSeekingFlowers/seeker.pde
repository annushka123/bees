class Vehicle {
    ...
    Vehicle(float x, float y, int swarm) {
        ...
        maxspeed = 9 + random(2);  // Increase base speed
        maxforce = 0.5 + random(0.1);  // Increase maximum steering force
...
void applyBehaviors(ArrayList<Vehicle> vehicles) {
    PVector flowerTarget = findNearestFlower();
    if (flowerTarget != null) {
        PVector seekFlower = seek(flowerTarget);
        applyForce(seekFlower);
    }
    ...
    PVector findNearestFlower() {
    Flower nearestFlower = null;
    float nearestDistance = Float.MAX_VALUE;

    for (Flower flower : flowers) {
        float distance = PVector.dist(position, new PVector(flower.x, flower.y));
    ...
    if (nearestFlower != null) {
        return new PVector(nearestFlower.x, nearestFlower.y);
    } else {
        return null;
    }
}
