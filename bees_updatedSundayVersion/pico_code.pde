...
void receive(byte[] data) {
  if (data.length == 28) {  // 7 integers * 4 bytes each = 28 bytes
    ...
    if (imuId >= 0 && imuId < 4) {  // Ensure imuId is within the valid range
      ...
      mappedAccelX[3] = map(accelX[3], -8000, 8000, -1.9, 1.9);
      //println("Pico Mapped; ", mappedAccelX[3]);
      mappedAccelY[3] = map(accelY[imuId], -8000, 8000, -1.5, 1.5);
      ...
      mappedX[imuId] = constrain(map(x[imuId], -250, 250, 0, width), 0, width);  // Adjusted range and constrained
      mappedY[imuId] = constrain(map(y[imuId], -250, 280, 0, height), 0, height); // Adjusted range and constrained
      mappedZ[imuId] = map(z[imuId], -120, 120, -1, 1);
