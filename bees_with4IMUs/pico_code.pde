...
void receive(byte[] data) {
  if (data.length == 28) {  // 7 integers * 4 bytes each = 28 bytes
    ...
    if (imuId >= 0 && imuId < 4) {  // Ensure imuId is within the valid range
      ...
      mappedAccelX[imuId] = map(accelX[imuId], -8000, 8000, -0.5, .5);
      ...
      mappedX[imuId] = constrain(map(x[imuId], -200, 200, 0, width), 0, width);  // Adjusted range and constrained
      mappedY[imuId] = constrain(map(y[imuId], -200, 200, 0, height), 0, height); // Adjusted range and constrained
      
      for(int i=0; i < 4; i++) {
      println("mappedX; ", mappedX[i], "mappedY; ", mappedY[i]);
      }
