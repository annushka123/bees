class Flower {
    ...
    Flower(float x, float y, int id) {
        ...
        this.y = y;
        this.id = id;
        baseColor = color(random(30, 255), random(30, 250), random(30, 250)); // Random RGB color
    ...
    void update() {
        ...
        y += directionY * sideToSideAmplitudeY * 0.05;
        
        if (x < 200 || x > width-200) {
            directionX *= -1;  // Reverse direction if hitting the boundary
            x = constrain(x, 200, width-200);  // Constrain x within the window
        }
        
            if (y < 200 || y > height -200) {
            directionY *= -1;  // Reverse direction if hitting the boundary
            y = constrain(y, 200, width-200);  // Constrain x within the window
        }
    }
        ...
        float targetAmplitudeX = map(abs(newAdditionalRotationSpeedX), 0, 3, 20, 50);
        float targetAmplitudeY = map(abs(newAdditionalRotationSpeedY), 0, 3, 20, 50);
